...
public:
    ...
    int getQueueAndDel(T &value) { // 获取队列数据后，并且 删除 ？ 为了不混乱
        ...
        while (work && saveQueue.empty()) {
            // 如果是工作专题 并且 队列里面没有数据，我就阻塞这这里睡觉
            pthread_cond_wait(&cond, &mutex); // 没有数据就睡觉（C++线程的内容）
        }

        if (!saveQueue.empty()) { // 如果队列里面有数据，就进入此if
            // 取出队列的数据包 给外界，并删除队列数据包
            value = saveQueue.front();
            saveQueue.pop(); // 删除队列中的数据
    ...
    */
    ...
    void clear(){
        ...
        unsigned int size = saveQueue.size();

        for (int i = 0; i < size; ++i) {
            //循环释放队列中的数据
            T value = saveQueue.front();
            if(releaseCallback){
                releaseCallback(&value); // 让外界去释放堆区空间
            }
            saveQueue.pop(); // 删除队列中的数据，让队列为0
        }
    ...
    */
     ...
     */
     void sync() {
         ...
         syncCallback(saveQueue); // 函数指针 具体丢包动作，让外界完成
